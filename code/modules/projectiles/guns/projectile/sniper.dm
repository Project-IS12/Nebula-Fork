...
/obj/item/gun/projectile/heavysniper/handle_post_fire(mob/user, atom/target, var/pointblank=0, var/reflex=0)
	...
	if(user && user.skill_check(SKILL_WEAPONS, SKILL_PROF))
		attack_self(user)//Immediately eject it if you're skilled enough.
		spawn(1)
			attack_self(user)
...
/obj/item/gun/projectile/heavysniper/attack_self(mob/user)
	bolt_open = !bolt_open
	if(bolt_open)
		to_chat(user, "<span class='notice'>You work the bolt open[chambered ? ", ejecting \the [chambered]" : ""]!</span>")
		unload_shell()
